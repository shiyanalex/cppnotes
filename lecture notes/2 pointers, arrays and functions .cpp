...
когда запускаетя программа в памяти выделяются [ data | text | stack 4/8 mb]

data - static memory (поскольку она ограничена то она может переполниться stack overflow)
text - текст программы
stack - переменные заполняются и потом снимаются (дата со стеком не пересекаются)
		через new можно выделить динамическую память 

bss = Static data (uninitialized) / data initialized
text = instructions  
stack = local vars 
heap = malloc 
	...
	void f(){
		...
		x++;
		f(); 
	}   // 174691*32байта = 4 мегабайт  

	int x = 5;
	int* p = &x; //int* - pointer & - to get address  
	std::cout << x << std::endl;
	std::cout << &x << std::endl;
	std::cout << p << std::endl;
	std::cout << &p << std::endl;
...
====================================
	...
	int* p = a;
	std::cout << p[2] << std::endl;	// показали что массив и указатель практически одно и тоже 
...
=====================================================================
	...
	push_back добавляет копию объекта (или обеспечивает перемещение, если возможно), а 
